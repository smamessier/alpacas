...
import alpacas.Analysis._
import alpacas.Modelling.{_, given _}
import InputDistribution._
  
type TrimValue = Double

/** Engine component model.
 * Three different failure rates depending on
 *   the trimming percentage applied to the engine:
...
class CPU(lambda: Double) extends Component {
  ...
  /** True iff the CPU is running.
    * Depends on intrinsic failure state and received power. 
    */
 ...
 * @param opposite returns the index of the opposite engine
 * 
 * Makes trimming decision 
 * based on canProduceThrustSensed.
 * 
...
class TrimFunction(
  ...
  assertions {
    ...
    // Distance between min and max failed engine IDs,
    // assuming they are spread on a circle.
    cordDist :=
      (maxFailedId - minFailedId) min
      (minFailedId - maxFailedId + nofEngines)
      ...
      v :=
        If(hasCpu) Then {
          If(failedCount === 0) Then {
            ...
            // single engine loss
            If (
              !canProduceThrust(i) || !canProduceThrust(opposite(i))
            ) Then {
            ...
            If (cordDist === nofEngines/2) Then {
              ...
              // then engines are opposite
              // and thrust is already symmetric
              If (
                !canProduceThrust(i) || !canProduceThrust(opposite(i))
              ) Then {
                // shutdown failed and opposite engine
                -100.0
              } Else {
                // reallocate missing thrust to other engines
                200.0/(nofEngines - 2)
              ...
              // 2 failures but less than 8 engines, do not handle
              0.0
            }
          } Else {
 ...
 *  Senses the current failure state of an engine.
 *  @param lambda failure rate of the sensor
 *  @param optimistic controls the default value
 ...
 * The produced thrust must be symmetric and total base 
 * thrust must lie in interval [-margin, +margin]
 *  */
class StableLiftObserver(
  ...
  // true iff the failure condition is not reached
  // at the current step.
  val isOk = OutFlow[Boolean]

  // true iff all engines and their opposites have
  // the same base thrust value
  private val symmetricThrust =
    baseThrustValues.zipWithIndex.map {
      case (t, i) => t === baseThrustValues(opposite(i))
    }.reduce(_ && _)

  // sum of all base thrusts for all engines
  private val totalBaseThrust =
    baseThrustValues.fold(0.0:Expr[TrimValue])(_ + _)
...
/** Holds configuration parameter for an 
 *  engine + battery configuration. 
 */
...
type Sensors = Vector[Sensor]
type Batteries = Vector[Battery]
type Wiring = (Engines, Sensors, Batteries) => Assertions
...
class GenericPowertrain(
  ...
  assertions {
    ...
    engines.map(_.trimValue) :=
      function.trimValue

    function.canProduceThrust :=
      sensors.map(_.canProduceThrustTransmitted)

    // engines to sensors
    sensors.map(_.canProduceThrustSensed) :=
      engines.map(_.canProduceThrust)

    // engines to observer mapping
    observer.baseThrustValues :=
      engines.map(_.baseThrustValue)
  }
}
 ...
 * @param engineIdtoBatteryIds 
 *        returns the pair of battery Ids for an engine Id.
 * @param sensorIdToBatteryIds 
 *        returns the pair of battery Ids for a sensor Id.
 *  */
def genericWiring(
  engineIdtoBatteryIds: Int => (Int, Int),
  sensorIdToBatteryIds: Int => (Int, Int),
): Wiring =
  (engines: Engines, sensors: Sensors, batteries: Batteries) => {
    ...
    for {
      ...
      engines(i).isPowered :=
        batteries(eb1).power || batteries(eb2).power

      sensors(i).isPowered :=
        batteries(sb1).power || batteries(sb2).power
    }
  }
...
/** The main design space exploration loop. */
object DesignSpaceExploration extends App {
  ...
  val systems = for {
    engineParams <- List(
      EngineParameters(
        nofEngines = 6,
        lambda0 = 1E-5, lambda1 = 1E-4, lambda2 = 2E-4,
        lambdaBattery = 1E-5),
      EngineParameters(
        nofEngines = 8,
        lambda0 = 2E-5, lambda1 = 1E-4, lambda2 = 2E-4,
        lambdaBattery = 1E-5),
      EngineParameters(
        nofEngines = 10,
        lambda0 = 2.5E-5, lambda1 = 1E-4, lambda2 = 2E-4,
        lambdaBattery = 1E-5),
    )
  ...
  for {
    ...
    mcs <- minimalCutSetsBFS(
      model,
      system.observer.isOk,
      3 // maximum sequence order bound for exploration
    )
    ur <- unreliability(
      model,
      system.observer.isOk, // failure condition
      1E3, // mission time
      100000, // number of simulations
      8, // number of batches (use number of logical cores of computer)
    )
  } {
    ...
    val nofmcs1 = mcs1.count(_.events.size == 1)
  ...
  // produce table for section 7 of the paper
